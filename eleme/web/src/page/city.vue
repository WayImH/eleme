<template>
  	<div class="city_container">
        ...
        <form class="city_form">
            ...
            <div>
                <input type="button" name="submit" class="city_submit input_style"  value="提交" @click="find()">
            </div>
        </form>
        <header  class="pois_search_history">搜索历史</header>
        <ul class="getpois_ul" v-for="item in list" @click="jump(item)">
            <li>
                <h4 class="pois_name ellipsis">{{item.title}}</h4>
                <p class="pois_address ellipsis">{{item.address}}</p>
            </li>
        </ul>
        <footer class="clear_all_history" v-if="list.length">清空所有</footer>
...
<script>
    export default{
		...
		jump(item){
			...
			var position = item.location.lat+","+item.location.lang;
